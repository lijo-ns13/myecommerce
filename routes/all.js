    ...
    try {
        ...
        if (coupon) {
            ...
            if (purchaseAmount < coupon.minPurchaseAmount) {
                return res.json({ valid: false, message: `Minimum purchase amount is $${coupon.minPurchaseAmount}.` });
            }
...
router.post('/validate-coupon', async (req, res) => {
    ...
    try {
        ...
        if (foundCoupon.isDeleted || foundCoupon.startDate > currentDate || foundCoupon.endDate < currentDate) {
        ...
        // Coupon is valid
        return res.json({ success: true, message: 'Coupon is valid!', discountType: foundCoupon.discountType, discountValue: foundCoupon.discountValue });
